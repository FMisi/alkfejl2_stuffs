...
namespace Kisallatok.DAO
{
    internal class DaoAdoImpl : Dao
    {
        ...
        public bool DeleteKisallat(string kisallatnev)
        {
            // Kerdes a felhasznalonak, hogy biztosan torolni akarja-e
            DialogResult result = MessageBox.Show($"Biztosan torolni szeretned a {kisallatnev} nevu kisallatot?",
                                                  "Torles megerositese",
            ...
            {
                // Ha a felhasznalo nem akar torolni, akkor kilepunk a metodusbol
                return false;
            }

            // Ha a felhasznalo rakatintott az 'Igen' gombra, akkor folytatjuk a torlest
            using SQLiteConnection connection = new SQLiteConnection(CONN_STRING);
            connection.Open();

            string query = @"DELETE FROM Kisallatok WHERE Nev = @Nev";
            using SQLiteCommand command = new SQLiteCommand(query, connection);
            command.Parameters.AddWithValue("@Nev", kisallatnev); // A nev parameter hozzaadasa
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    MessageBox.Show($"A kisallat {kisallatnev} sikeresen torolve lett!");
                    return ret;
                }
                else
                {
                    MessageBox.Show($"Nem tortent torles, a(z) {kisallatnev} nevvel nem talalhato kisallat.");
                    return false;
                }
            }
            catch (Exception ex)
        ...
        {
            // Kerdes a felhasznalonak, hogy biztosan torolni akarja-e
            DialogResult result = MessageBox.Show($"Biztosan torolni szeretned a {kategoriaNev} kategoriat?",
                                                  "Torles megerositese",
                                                  MessageBoxButtons.YesNo,
                                                  MessageBoxIcon.Question);
            ...
            using SQLiteCommand command = new SQLiteCommand(query, connection);
            command.Parameters.AddWithValue("@Kategoria", kategoriaNev); // A kategoria nev parameter hozzaadasa
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    MessageBox.Show($"{kategoriaNev} Kategoria sikeresen torolve lett!");
                    return ret;
                }
                else
                {
